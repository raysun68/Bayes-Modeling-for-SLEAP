...
decay_rate = 0.8, decay_ang = 0.5, verbose = FALSE) {
  ...
  for (f in frames_seq) {
    ...
    for (ci in seq_len(nrow(combos))) {
      ...
      nose0 <- fill_with_prev(nose0, prev_nose0);   mid0  <- fill_with_prev(mid0,  prev_mid0);   tail0 <- fill_with_prev(tail0, prev_tail0)
      nose1 <- fill_with_prev(nose1, prev_nose1);   mid1  <- fill_with_prev(mid1,  prev_mid1);   tail1 <- fill_with_prev(tail1, prev_tail1)
...
combo_scores$score1[ci] <- avg_nll1 + motion_pen1 + w_ang * nll_ang1 + angle_pen_mid
    ...
    dist0_prev1 <- sqrt((combo_scores$mid0x - prev_mid1[1])^2 + (combo_scores$mid0y - prev_mid1[2])^2)
    dist1_prev0 <- sqrt((combo_scores$mid1x - prev_mid0[1])^2 + (combo_scores$mid1y - prev_mid0[2])^2)
  ...
  dplyr::mutate(
    ...
    valid_both <- combo_scores %>%
      dplyr::filter(!is.na(score0), !is.na(score1), score0 < threshold, score1 < threshold, mid_dist >= too_close_thresh, mid_sum0 / 3 <=  dist0_prev1, mid_sum1 / 3 <= dist1_prev0
)
    
    valid0 <- combo_scores %>% dplyr::filter(!is.na(score0), mid_dist > too_close_thresh / 2, score0 < threshold, mid_sum0 / 3 <=  dist0_prev1)
    
    valid1 <- combo_scores %>% dplyr::filter(!is.na(score1), mid_dist > too_close_thresh / 2, score1 < threshold, mid_sum1 / 3 <= dist1_prev0)
    # decision logic
...
if (f == r) {
  ...
  # Only update prev_* if they are reliable (prevents NAs from padding row)
  if (reliability_mid0)  prev_mid0  <- prev_mid0
  if (reliability_mid1)  prev_mid1  <- prev_mid1
  if (reliability_nose0) prev_nose0 <- prev_nose0
  if (reliability_nose1) prev_nose1 <- prev_nose1
  if (reliability_tail0) prev_tail0 <- prev_tail0
  if (reliability_tail1) prev_tail1 <- prev_tail1
}
