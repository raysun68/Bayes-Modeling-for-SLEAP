...
decay_rate = 0.8, decay_ang = 0.5, verbose = FALSE) {
  ...
  for (f in frames_seq) {
    ...
    for (ci in seq_len(nrow(combos))) {
      ...
      penalties0 <- mapply(function(mot, gap) if (!is.na(mot) && mot > (gap * motion_alpha)) (mot - gap * motion_alpha) else if(mot > max((gap - 1), 2) * motion_alpha) threshold / 2    else 0,
            dists0, gaps0)
      penalties1 <- mapply(function(mot, gap) if (!is.na(mot) && mot > (gap * motion_alpha)) (mot - gap * motion_alpha)  else if(mot > max((gap - 1), 2) * motion_alpha) threshold / 2   else 0
, dists1, gaps1)
