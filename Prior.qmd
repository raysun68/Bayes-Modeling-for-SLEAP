...
manual_angles_sd_only <- manual_wide %>%
  # compute vectors from Mid -> Nose and Mid -> Tail-base
  mutate(
  ...
  summarise(
    # compute R relative to fixed mean = pi
    R = sqrt(
      (mean(cos(angle_mid - pi), na.rm = TRUE))^2 +
      (mean(sin(angle_mid - pi), na.rm = TRUE))^2
    ),
    circ_sd_rad = sqrt(pmax(0, -2 * log(R))),
    circ_sd_deg = circ_sd_rad * 180 / pi
  )

circ_sd = manual_angles_sd_only$circ_sd_rad
...
decay_rate = 0.8, decay_ang = 0.5, verbose = FALSE) {
  ...
  for (f in frames_seq) {
    ...
    for (ci in seq_len(nrow(combos))) {
      ...
      nll_ang0 <- angle_nll(ang0, prior_mean = pi, prior_sd = circ_sd / 2) * decay_ang ^ (max(frame_gap_tail0, frame_gap_nose0, frame_gap_mid0) - 1)
      nll_ang1 <- angle_nll(ang1, prior_mean = pi, prior_sd = circ_sd / 2) * decay_ang ^ (max(frame_gap_tail1, frame_gap_nose1, frame_gap_mid1) - 1)
      
            # angle change penalty: penalize >90° flip in nose-tail direction
      angle_change0 <- NA_real_
      angle_change1 <- NA_real_

      prev_vec0 <- prev_nose0 - prev_tail0
      curr_vec0 <- nose0 - tail0
      prev_vec1 <- prev_nose1 - prev_tail1
      curr_vec1 <- nose1 - tail1

      if (!any(is.na(c(prev_vec0, curr_vec0)))) {
        cosang0 <- sum(prev_vec0 * curr_vec0) / (sqrt(sum(prev_vec0^2)) * sqrt(sum(curr_vec0^2)))
        cosang0 <- max(min(cosang0, 1), -1)
        angle_change0 <- acos(cosang0)
      }

      if (!any(is.na(c(prev_vec1, curr_vec1)))) {
        cosang1 <- sum(prev_vec1 * curr_vec1) / (sqrt(sum(prev_vec1^2)) * sqrt(sum(curr_vec1^2)))
        cosang1 <- max(min(cosang1, 1), -1)
        angle_change1 <- acos(cosang1)
      }
      angle_pen0 <- if (!is.na(angle_change0) && angle_change0 > pi / 2) 20 else 0
      angle_pen1 <- if (!is.na(angle_change1) && angle_change1 > pi / 2) 20 else 0


# --- combine into total scores ---
combo_scores$score0[ci] <- avg_nll0 + motion_pen0 + w_ang * nll_ang0 + angle_pen0
combo_scores$score1[ci] <- avg_nll1 + motion_pen1 + w_ang * nll_ang1 + angle_pen1
    ...
    valid_both <- combo_scores %>%
      dplyr::filter(!is.na(score0), !is.na(score1), score0 < threshold, score1 < threshold, mid_dist >= too_close_thresh, dist0_prev1 >= mid_sum0 / 3, dist1_prev0 >= mid_sum1 / 3)
    valid0 <- combo_scores %>% dplyr::filter(!is.na(score0), score0 < threshold, dist0_prev1 >= mid_sum0 / 3, dist0_prev1 >= pmin(motion_alpha, mid_sum0))
    valid1 <- combo_scores %>% dplyr::filter(!is.na(score1), score1 < threshold, dist1_prev0 >= mid_sum1 / 3, dist1_prev0 >= pmin(motion_alpha, mid_sum1))
    # decision logic
