...
library(tidyr)
# Change 1
coords = read_csv("predictions_21_19_29.csv")
str(coords)
```
...
```{r, message  = FALSE}
# Change 2
manual = read_csv("predictions_manual_21_19_29.csv")[1:240, ]
manual$instance =  rep(rep(0:1, each = 3), 40)
saveRDS(manual, "manual_21_19_29.rds")
```
...
decay_rate = 0.8, decay_ang = 0.5, verbose = FALSE) {
 ...
 frames_seq <- r:max(coords_sort$frame)
  ...
  if (!is.na(mot) && !is.na(gap) && mot > gap * motion_alpha) {
    mot - gap * motion_alpha
  } else if (!is.na(mot) && !is.na(gap) &&
             mot > max((gap - 1), 2) * motion_alpha) {
    threshold / 2
  } else {
    0
  }
...
```{r}
res_1000 <- res %>% filter(frame >= 7200, frame <= 7500)
...
saveRDS(res, file = "Tracked_21_19_29.rds")
```
